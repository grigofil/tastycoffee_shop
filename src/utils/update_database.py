...
- pandas: for processing data (optional)
...
def main():
    ...
    try:
        ...
        global API_TOKEN
        token_file = Path("api_token.txt")
        if token_file.exists():
            with open(token_file, "r") as f:
                API_TOKEN = f.read().strip()
            print(f"Загружен токен из файла: {API_TOKEN[:10]}...")
            
            # Попытка получить данные с использованием токена
            if REQUESTS_AVAILABLE:
                try:
                    products_data = get_data_with_token()
                    print("Данные успешно получены с использованием сохраненного токена.")
                except Exception as e:
                    print(f"Ошибка при использовании сохраненного токена: {e}")
                    API_TOKEN = None
        
        # Если токен не сработал, пробуем авторизоваться
...
def get_data_with_token():
    """Получение данных с использованием сохраненного токена"""
    if not API_TOKEN:
        raise Exception("Токен не установлен")
        
    print("Получение данных с использованием сохраненного токена...")
    s = requests.Session()
    s.headers.update({
    ...
    rsp = s.get('https://api.tastycoffee.ru/api/v1/catalog/products')
...
def get_data_with_requests():
    ...
    global API_TOKEN
    API_TOKEN = response_data['data']['access_token']
    
    # Сохранение токена в файл для последующего использования
    with open("api_token.txt", "w") as f:
        f.write(API_TOKEN)
    print(f"Токен сохранен в файл api_token.txt")
    
    return get_data_with_token()
...
def get_data_with_urllib():
    ...
    # Если первый способ не сработал, пробуем JSON
    if login_response['status'] != 200:
        print("Пробуем вход с JSON...")
        login_response = make_request(login_url, method='POST', json_data=login_data)
        print(f"Попытка входа с JSON: {login_response['status']}")
...
def process_with_pandas(products_data):
    ...
    items = df1[['id_offer','name_offer','info.mini_description','id','price','discount','card_product', 'weight', 'type_offer']].rename(
...
def process_without_pandas(products_data):
    ...
    for item in items:
        conn.execute(
            'INSERT INTO items (id, name, description, category_id, price, discount, image_id, weight, is_hidden) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
            (item['id'], item['name'], item['description'], item['category_id'], item['price'], 
             item['discount'], item['image_id'], item['weight'], item['is_hidden'])
        )
